...
function renderStandings(standings) {
  ...
  const driverImages={
    'norris':'landoNorris.png',
    'piastri':'oscar_piastri.png',
    'verstappen':'maxVerstapenn1.wpeg',
    'russell':'georgeRussell.png',
    'leclerc':'charles2new.png',
    'hamilton':'lewisHamilton.png',
    'antonelli':'kimiAntonelli.png',
    'tsunoda':'yukiTsunodza.png',
    'albon':'alexanderAlbon.png',
    'sainz':'carlosSainz.jpg',
    'alonso':'fernandoAlonso.jpeg',
    'stroll':'lanceStroll.jpeg',
    'gasly':'pierreGasly.jpeg',
    'ocon':'estebanOcon.jpeg',
    'hulkenberg':'nicoHulkenberg.jpeg',
    'magnussen':'kevinMagnussen.png',
    'bottas':'valtteriBottas.png',
    'zhou':'zhouGuanyu.png',
    'ricciardo':'danielRicciardo.png',
    'lawson':'liamLawson.jpeg',
    'hadjar': 'isackHadjar.webp',
    'doohan': 'jackDoohan.webp',
    'bearman': 'oliverBerman.webp',
    'colapinto': 'francoColapento.webp',
    'bortoleto': 'gabrielBortoleto.webp'
}

  // flag mapping
  const flagMap = {
    'Australian': 'australia.webp',
    'Argentine' : 'argentina.webp',
    'British': 'ukflag.webp',
    'Brazilian': 'brazil.webp',
...
function renderComparison(data) {
  ...
  container.innerHTML = `
    <div class="bg-[#1a1b1e] rounded-2xl p-8 shadow-2xl">
      ...
      <!-- Differences Summary -->
      <div class="bg-[#1a1b1e] rounded-xl p-6 text-center border border-gray-700">
...
document.addEventListener('DOMContentLoaded',async()=>{
    ...
    try{
        ...
        // ADD THIS HERE
        document.getElementById('refreshStandings').addEventListener('click',async()=>{
...
async function fetchPredictions() {
    ...
    try {
        ...
        const response = await fetch('http://localhost:8000/api/predict-podium');
        ...
        if (result.success) {
            document.getElementById('predictStatus').textContent=`Predicted for: ${result.race} ${result.year}`;
            document.getElementById('predictStatus').className='text-gray-400 text-sm mt-4';
        ...
        predictStatus.textContent = 'Error: Make sure server is running';
...
function displayPredictions(predictions) {
    ...
    predictions.forEach((prediction,index) => {
        ...
        const medal=index===0?'P1':index===1?'P2':index===2?'P3':'';
        ...
        html+=`
            <div class="grid grid-cols-12 gap-4 p-3 hover:bg-[#1a1b1f] transition ${rankBg}">
                ...
                <div class="col-span-5">
                    ...
                    <p class="text-gray-400 text-xs">${prediction.TeamName||''}</p>
                </div>
                <div class="col-span-3 text-right">
                    <span class="text-base font-bold text-white">P${prediction.PredictedPosition}</span>
                </div>
            </div>
        `;
    });
